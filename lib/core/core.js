...
// Factory that produces packets from DOM elements
nx.factory.packet = function( e ) {
	return {
		speed: parseInt(e.attr('speed')),
		type: e.attr('type'),
		data: nx.toHTML(e.html())
	}
};
...
nx.circuit.add = function( circuit ) {
	...
	if (nx.circuits[circuit.x + '.' + circuit.y]) return nx.error('Manipulation Error', 'Circuit already exists at this point', {x:circuit.x, y:circuit.y, add:true});
...
nx.circuit.remove = function( x, y ) {
	...
	if (!c) return nx.error('Manipulation Error', 'No circuit to remove at this point', {x:x, y:y, remove:true});
	c.draw.remove();
